/*
 * Novalnet backend configuration script
 *
 * @author    Novalnet AG
...
$(document).ready(function() {
    ...
    $('input[name="aNovalnetConfig[iDueDatenovalnetsepa]"],input[name="aNovalnetConfig[iDueDatenovalnetinvoice]"],input[name="aNovalnetConfig[iDueDatenovalnetprepayment]"], input[name="aNovalnetConfig[iDueDatenovalnetbarzahlen]"]').keydown(function (event) {
    ...
    $("#sPaymentActionnovalnetcreditcard,#sPaymentActionnovalnetinvoice,#sPaymentActionnovalnetsepa,#sPaymentActionnovalnetpaypal").change(function(){
     ...
     $("#blCreditcardInlineForm").click(function(){
         if ($(this).prop("checked") == false){
             $("#sCreditcardDefaultInput").val($("#sCreditcardInputForm").val());
            }
     });
...
function getMerchantConfigs(novalnetActivationKey)
{
    ...
    var formurl  = shopurl+"index.php?cl=novalnetadmincontroller&fnc=getMerchantDetails&stoken="+stoken;
            ...
            if(resultData.details == 'true') {
                ...
                if (response.status == '100') {
                    if (response.vendor != undefined && response.product != undefined) {
                        ...
                        $('#novalnet_clientkey').val(response.client_key);
                        ...
                        if (novalnetSavedTariff != '') {
                            ...
                            $('#novalnet_tariffid').val($.trim(Object.keys(response.tariff)[0])).attr("selected", "selected");
                        }
                    }
            } else {
                if (response.status == '106') {
                    ...
                    $('#novalnet_tariffid').val('');
                    $('#novalnet_clientkey').val('');
                }
            }
