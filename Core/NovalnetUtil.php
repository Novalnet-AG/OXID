...
class NovalnetUtil
{
    ...
    /**
      ...
      * @var string
      */
    static $sNovalnetVersion = '13.0.0';
     ...
     * @var string
     */
    static $sEndPoint = 'https://payport.novalnet.de/v2/';

    static $aUnPaidPayments = [
        'INVOICE', 'PREPAYMENT', 'CASHPAYMENT', 'MULTIBANCO'
    ];
    ...
    public static function getBasketDetails($oBasket, $shippingcost, $dAmount = false)
    {
        ...
        foreach($oBasket->getContents() as $basketItem) {
            ...
            if ($dAmount) {
                ...
                $label = $basketItem->getTitle().' '. $basketItem->getAmount(). ' x '. Registry::getConfig()->getActShopCurrencyObject()->name. sprintf('%0.2f', $amount);
    ...
    public static function importPaymentDetails(&$aRequest, $oOrder, $aData)
    {
        ...
        $aPaymentDataBookingDetails = [
          'iban', 'account_holder', 'bic', 'pan_hash', 'unique_id', 'wallet_token'
        ];
        $aTransactionBookingDetails = [
          'enforce_3d', 'create_token'
        ];
    ...
    public static function getCartInfo($dPaymentCost)
    {
        ...
        return ['line_items' => $articleDetails, 'items_shipping_price' => NovalnetUtil::formatCost($dDeliveryCost->getBruttoPrice())];
    }
            ...
            if (!empty($aInstalment['cycles_executed'])) {
                ...
                for ($iCycle = 1; $iCycle <= $aInstalmentDetails['instalment_total_cycles']; $iCycle++) {
                    ...
                    if (1 < $iCycle && $iCycle < $aInstalmentDetails ['instalment_total_cycles']) {
                        $aInstalmentDetails ['instalment' . $iCycle]['amount'] = self::formatCurrency($aInstalment['cycle_amount'], $aData['transaction']['currency']);
                        ...
                        $aInstalmentDetails ['instalment' . $iCycle]['amount'] = self::formatCurrency($last_cycle, $aData['transaction']['currency']);
                    }
                    ...
                    foreach (array('instalment_cycles_executed' => 'cycles_executed','due_instalment_cycles' => 'pending_cycles','amount' => 'cycle_amount') as $key => $value) {
                        if (!empty($aInstalment[$value])) {
                            if ($key == 'amount') {
                                $amount = self::formatCurrency($aInstalment[$value], $aData['transaction']['currency']);
    ...
    /**
     * Forms store for cashpayment orders
     *
     * @param array $aData
     *
     * @return string
     */
    public static function getStoreDetails($aData)
    {
        $aNovalnetComments = [];
        $oCountry = oxNew(\OxidEsales\Eshop\Application\Model\Country::class);
        $aComments = [];
        $aCountrySet    = [];
        $aNovalnetComments[] = ['NOVALNET_SLIP_EXPIRY_DATE' => [$aData['transaction']['due_date']]];
        if (isset($aData['transaction']['nearest_stores'])) {
            foreach ($aData['transaction']['nearest_stores'] as $nearestStore => $stores) {
                foreach (['store_name', 'street', 'city', 'zip', 'country_code'] as $value) {
                    if($value == 'country_code' ) {
                        if(! isset($aCountrySet[$stores[$value]]) ) {
                            $sCountryId = new Field(oxNew(Country::class)->getIdByCode($stores[$value]));
                            if ($oCountry->load($sCountryId)) {
                                $aCountrySet[$stores[$value]] = $oCountry->oxcountry__oxtitle->rawValue;
                            }
                        }
                        if(! empty($aCountrySet[$stores[$value]]) ) {
                            $stores[$value] = $aCountrySet[$stores[$value]];
                        }
                    }
                    array_push($aComments, $stores[$value]);
                    array_push($aComments, '<br>');
                }
                array_push($aComments, '<br>');
            }
            $aNovalnetComments[] = ['NOVALNET_STORE_NEAR_YOU' => [null]];
            $aNovalnetComments[] = ['GENERAL' => [implode('', $aComments)]];
        }
        return $aNovalnetComments;
    }
