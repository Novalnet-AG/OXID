    ...
    */
     ...
     */
     public function finalizeOrder(\OxidEsales\Eshop\Application\Model\Basket $oBasket, $oUser, $blRecalculatingOrder = false)
     {
        ...
        $aDynValue = $this->_oNovalnetUtil->oSession->getVariable('dynvalue');
        ...
        if (in_array($this->sCurrentPayment, $this->_oNovalnetUtil->aRedirectPayments) && empty($this->_oNovalnetUtil->oConfig->getRequestParameter('tid')))
           $this->_oNovalnetUtil->doPayment($this);
    ...
    private function _logNovalnetTransaction()
    {
        ...
        $aDynValue = $this->_oNovalnetUtil->oSession->getVariable('dynvalue');
        if ((in_array($this->_aNovalnetData['key'], ['27', '37', '40', '41','59' ]) || ($this->_aNovalnetData['key'] == '34' && isset($aRequest['payment_ref']))) || ($this->_aNovalnetData['key'] == '6' && (isset($aDynValue['novalnet_cc_do_redirect']) && $aDynValue['novalnet_cc_do_redirect'] != '1'))) {
        ...
        if (!in_array($this->_aNovalnetData['key'], [ '27', '59', '41', '40']) && $this->_aNovalnetData['status'] == 100 && !in_array($this->_aNovalnetData['tid_status'], [86, 85, 90, 98, 99])) {
            if (!in_array($this->_aNovalnetData['tid_status'], [86, 85, 90, 98, 99]) && $this->_oNovalnetUtil->getNovalnetConfigValue('iShopType' . $this->sCurrentPayment) != '2') // verifying onhold status
