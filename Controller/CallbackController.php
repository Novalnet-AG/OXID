...
class CallbackController extends FrontendController
{
    ...
    protected $aPayments         = ['CREDITCARD', 'INVOICE_START', 'DIRECT_DEBIT_SEPA', 'GUARANTEED_INVOICE', 'GUARANTEED_DIRECT_DEBIT_SEPA', 'PAYPAL', 'ONLINE_TRANSFER', 'IDEAL', 'EPS', 'GIROPAY', 'PRZELEWY24','CASHPAYMENT','TRUSTLY','ALIPAY','WECHATPAY','ONLINE_BANK_TRANSFER'];

    /** @Array Type of Chargebacks available - Level : 1 */
    protected $aChargebacks      = ['RETURN_DEBIT_SEPA', 'CREDITCARD_BOOKBACK', 'CREDITCARD_CHARGEBACK', 'PAYPAL_BOOKBACK', 'PRZELEWY24_REFUND', 'ALIPAY_REFUND','WECHATPAY_REFUND','TRUSTLY_REFUND','REFUND_BY_BANK_TRANSFER_EU', 'REVERSAL','CASHPAYMENT_REFUND', 'GUARANTEED_INVOICE_BOOKBACK', 'GUARANTEED_SEPA_BOOKBACK'];
    ...
    protected $aPaymentGroups    = [   'novalnetcreditcard'      => [ 'CREDITCARD', 'CREDITCARD_BOOKBACK', 'CREDITCARD_CHARGEBACK', 'CREDIT_ENTRY_CREDITCARD','DEBT_COLLECTION_CREDITCARD', 'SUBSCRIPTION_STOP', 'SUBSCRIPTION_REACTIVATE'],
                                        ...
                                        'novalnetalipay'     => [ 'ALIPAY', 'ALIPAY_REFUND'],
                                        'novalnetwechatpay'     => [ 'WECHATPAY', 'WECHATPAY_REFUND'],
                                        'novalnettrustly'     => [ 'TRUSTLY', 'TRUSTLY_REFUND'],
                                        'novalnetonlinebanktransfer'     => [ 'ONLINE_BANK_TRANSFER', 'REFUND_BY_BANK_TRANSFER_EU', 'ONLINE_TRANSFER_CREDIT', 'REVERSAL', 'DEBT_COLLECTION_DE', 'CREDIT_ENTRY_DE'],
    ...
    private function _validateCaptureParams()
    {
        ...
        if (($sIpAddress != $sIpAllowed) && empty($this->_blProcessTestMode)) {
            $this->_displayMessage('Unauthorized access from the IP [' . $sIpAddress . ']');
            return false;
        }
        ...
        if (!is_numeric($this->_aCaptureParams['status']) || $this->_aCaptureParams['status'] <= 0) {
            $this->_displayMessage('Status (' . $this->_aCaptureParams['status'] . ') is not valid');
            return false;
        }

        foreach (['signup_tid', 'tid_payment', 'tid'] as $sTid) {
            if (!empty($this->_aCaptureParams[$sTid]) && !preg_match('/^\d{17}$/', $this->_aCaptureParams[$sTid])) {
                $this->_displayMessage('Invalid TID [' . $this->_aCaptureParams[$sTid] . '] for Order');
                return false;
            }
        }
        return true;
    ...
    private function _processNovalnetCallback()
    {
        ...
        $dCaptureAmount = $this->_aCaptureParams['amount']/100;
        $oConfig   = \OxidEsales\Eshop\Core\Registry::getConfig();
        $this->_oLang   = \OxidEsales\Eshop\Core\Registry::getLang();

        $dFormattedAmount = $this->_oLang->formatCurrency($dCaptureAmount, $oConfig->getCurrencyObject($this->_aCaptureParams['currency'])) . ' ' . $this->_aCaptureParams['currency'];
        ...
        if ($iPaymentTypeLevel === 0) {
            if ($this->_aCaptureParams['payment_type'] == 'PAYPAL' && in_array($this->aOrderDetails['GATEWAY_STATUS'], [85, 90])) {
                 if ($this->aOrderDetails['GATEWAY_STATUS'] == '90' && $this->_aCaptureParams['tid_status'] == '85') {
                    $sNovalnetComments = $sLineBreak .'The transaction status has been changed from pending to on hold for the TID:'.$this->_aCaptureParams['shop_tid'].' on ' . date('Y-m-d H:i:s');

                } elseif(in_array($this->aOrderDetails['GATEWAY_STATUS'], [85, 90]) && $this->_aCaptureParams['tid_status'] == 100) {
                      $sTransactionConfirm = $sLineBreak .'The transaction has been confirmed on '.      date('Y-m-d H:i:s');
                      $sTransactionUpdate = $sLineBreak .'Transaction updated successfully for the TID: '.$this->_aCaptureParams['shop_tid'].' with the amount '.$dFormattedAmount.' on '. date('Y-m-d'). ' & ' .date('H:i:s');
                      $sNovalnetComments = ($this->aOrderDetails['GATEWAY_STATUS'] == 85) ? $sTransactionConfirm : $sTransactionUpdate;
                      $this->_oDb->execute('UPDATE oxorder SET OXPAID = "' . date('Y-m-d H:i:s') . '" WHERE OXORDERNR ="' . $this->aOrderDetails['ORDER_NO'] . '"');
                }

                // Update paypal transaction ID
                if (!empty($this->_aCaptureParams['paypal_transaction_id']) && !empty($this->aOrderDetails['MASKED_DETAILS'])) {
                    $aMaskedDetails = serialize([
                        'paypal_transaction_id' => $this->_aCaptureParams['paypal_transaction_id'],
                        'tid'                   => $this->_aCaptureParams['shop_tid'],
                    ]);
                }
                $this->_oDb->execute('UPDATE novalnet_transaction_detail SET GATEWAY_STATUS = "' . $this->_aCaptureParams['tid_status'] . '", MASKED_DETAILS = "'.$aMaskedDetails.'" WHERE ORDER_NO ="' . $this->aOrderDetails['ORDER_NO'] . '"');
                $this->_oDb->execute('UPDATE oxorder SET NOVALNETCOMMENTS = CONCAT(IF(NOVALNETCOMMENTS IS NULL, "", NOVALNETCOMMENTS), "' . $sNovalnetComments . '") WHERE OXORDERNR ="' . $this->aOrderDetails['ORDER_NO'] . '"');
                $sNovalnetComments = $this->_sendMail($sNovalnetComments) . $sNovalnetComments;
                $this->_displayMessage($sNovalnetComments);

             }  elseif($this->_aCaptureParams['payment_type']=='PRZELEWY24' && $this->aOrderDetails['GATEWAY_STATUS'] == '86' && $this->_aCaptureParams['tid_status'] == '100') {
                $sNovalnetComments = $sLineBreak .'The transaction has been confirmed on '.      date('Y-m-d H:i:s');
                $this->_oDb->execute('UPDATE oxorder SET OXPAID = "' . date('Y-m-d H:i:s') . '" WHERE OXORDERNR ="' . $this->aOrderDetails['ORDER_NO'] . '"');
                ...
                $this->_displayMessage($sNovalnetComments);
                    ...
                    $sNovalnetComments = 'The transaction has been canceled due to: ' . $this->_oNovalnetUtil->setPaygateError($this->_aCaptureParams);
            ...
            }  elseif (in_array($this->_aCaptureParams['payment_type'], ['INVOICE_START', 'GUARANTEED_INVOICE', 'GUARANTEED_DIRECT_DEBIT_SEPA', 'DIRECT_DEBIT_SEPA', 'CREDITCARD','TRUSTLY']) &&  in_array($this->_aCaptureParams['tid_status'], ['91', '98', '99', '100']) && in_array($this->aOrderDetails['GATEWAY_STATUS'], ['75','91','98','99'])) {
                     $sNovalnetComments = '';
                     $sMessage = '';
                    if (in_array($this->aOrderDetails['GATEWAY_STATUS'], ['75', '91', '98', '99']) && $this->_aCaptureParams['tid_status'] == '100') {
                        $sNovalnetComments .= '<br>The transaction has been confirmed on '. date('Y-m-d H:i:s');
                    } elseif ($this->aOrderDetails['GATEWAY_STATUS'] == '75' && in_array($this->_aCaptureParams['tid_status'], ['91', '99'])) {
                       $sNovalnetComments .= '<br>The transaction status has been changed from pending to on hold for the TID:'. $this->_aCaptureParams['shop_tid']. ' on '. date('Y-m-d H:i:s').'<br>';
                    }
            ...
            elseif ($this->_aCaptureParams['status'] != '100' || !in_array($this->_aCaptureParams['tid_status'], ['100', '85', '86', '90', '91', '98', '99'])) {
                $this->_displayMessage('Status is not valid');
            } else {
                $this->_displayMessage('Payment type ( ' . $this->_aCaptureParams['payment_type'] . ' ) is not applicable for this process!');
            }
        } elseif ($iPaymentTypeLevel == 1 && $sPaymentSuccess) {
            if ($this->_aCaptureParams['payment_type'] == 'REVERSAL') {
                $sNovalnetComments = 'Chargeback executed for reversal of TID: ' . $this->_aCaptureParams['tid_payment'] . '  with the amount ' . $dFormattedAmount . ' on ' . date('Y-m-d'). ' & '. date('H:i:s') . '. The subsequent TID: ' . $this->_aCaptureParams['tid'];
            } elseif ($this->_aCaptureParams['payment_type']== 'RETURN_DEBIT_SEPA') {
                $sNovalnetComments = 'Chargeback executed for return debit of TID: ' . $this->_aCaptureParams['tid_payment'] . ' with the amount ' . $dFormattedAmount . ' on ' . date('Y-m-d'). ' & '. date('H:i:s') . '. The subsequent TID: ' . $this->_aCaptureParams['tid'];
            } else {
                $sNovalnetComments = 'Chargeback executed successfully for the TID: ' . $this->_aCaptureParams['tid_payment'] . ' with the amount ' . $dFormattedAmount . ' on ' . date('Y-m-d'). ' & '. date('H:i:s') . '. The subsequent TID: ' . $this->_aCaptureParams['tid'];
            }

            if (in_array($this->_aCaptureParams['payment_type'], ['CREDITCARD_BOOKBACK', 'PAYPAL_BOOKBACK', 'PRZELEWY24_REFUND', 'REFUND_BY_BANK_TRANSFER_EU', 'CASHPAYMENT_REFUND', 'GUARANTEED_INVOICE_BOOKBACK', 'GUARANTEED_SEPA_BOOKBACK','TRUSTLY_REFUND','ALIPAY_REFUND','WECHATPAY_REFUND'])) {
                $sNovalnetComments = 'Refund has been initiated for the TID: ' . $this->_aCaptureParams['tid_payment'] . ' with the amount ' . $dFormattedAmount.' .  New TID: '.$this->_aCaptureParams['tid'].' for the refunded amount .';
            }
            $sComments = $sLineBreak . $sNovalnetComments;
            ...
            if (in_array($this->_aCaptureParams['payment_type'], array('INVOICE_CREDIT','CASHPAYMENT_CREDIT','ONLINE_TRANSFER_CREDIT'))) {
                if (!isset($dPaidAmount) || $dPaidAmount < $dAmount) {
                    ...
                    $sNovalnetComments = 'Credit has been successfully received for the TID: ' . $this->_aCaptureParams['tid_payment'] . ' with amount ' . $dFormattedAmount .'on '.  date('Y-m-d').' & '.  date('H:i:s') .'. Please refer PAID order details in our Novalnet Admin Portal for the TID: ' . $this->_aCaptureParams['tid'];
                    $sComments = $sLineBreak . $sNovalnetComments;
                    $sUpdateSql = 'UPDATE oxorder SET NOVALNETCOMMENTS = CONCAT(IF(NOVALNETCOMMENTS IS NULL, "", NOVALNETCOMMENTS), "' . $sComments . '") WHERE OXORDERNR ="' . $this->aOrderDetails['ORDER_NO'] . '"';
                    ...
                    $this->_displayMessage($sNovalnetComments);
                     ...
                     $this->_displayMessage('Order already Paid');
                }
            } else {
                $sNovalnetComments = 'Credit has been successfully received for the TID: ' . $this->_aCaptureParams['tid_payment'] . ' with amount ' . $dFormattedAmount . ' on '.  date('Y-m-d').' & '.  date('H:i:s') .'. Please refer PAID order details in our Novalnet Admin Portal for the TID: ' . $this->_aCaptureParams['tid'];
                $sComments = $sLineBreak . $sNovalnetComments;
    ...
    private function _getOrderDetails()
    {
       ...
       $sSql  = 'SELECT trans.ORDER_NO, trans.TOTAL_AMOUNT, trans.REFUND_AMOUNT, trans.PAYMENT_ID, trans.GATEWAY_STATUS, trans.MASKED_DETAILS, o.OXLANG, o.OXPAYMENTTYPE FROM novalnet_transaction_detail trans JOIN oxorder o ON o.OXORDERNR = trans.ORDER_NO where trans.tid = "' . $this->_aCaptureParams['shop_tid'] . '"';
        ...
        if (strpos($aResult['OXPAYMENTTYPE'], 'novalnet') !== false && empty($aResult['novalnetcomments']) && in_array($this->_aCaptureParams['payment_type'], $this->aPayments)) {
           ...
           if (! in_array($this->_aCaptureParams['payment_type'], $this->aPaymentGroups[$aResult['OXPAYMENTTYPE']])) {
             $this->_displayMessage('Payment Type ['.$this->_aCaptureParams['payment_type'].'] is not valid.');
            }
        ...
        if (!empty($this->aOrderDetails['ORDER_NO'])) {
            ...
            if (!empty($iOrderNo) && $iOrderNo != $this->aOrderDetails['ORDER_NO']) {
                $this->_displayMessage('Order Number is not valid');
                return false;
            }
        }
     ...
     */
     private function _NotificationMessage()
     {
        ...
        $sSubject  = $this->_oLang->translateString('NOVALNET_CRITICAL_ERROR_MESSAGE1').$sShopName. sprintf($this->_oLang->translateString('NOVALNET_CRITICAL_ERROR_MESSAGE2'), $this->_aCaptureParams['shop_tid']);
    ...
    private function _transactionCancellation()
    {
         if ($this->_aCaptureParams['payment_type'] == 'TRANSACTION_CANCELLATION' && in_array($this->aOrderDetails['GATEWAY_STATUS'], ['75','85','91','98','99'])) {
           $sNovalnetComments = 'The transaction has been cancelled on '.date('Y-m-d H:i:s');
   ...
   /**
    ...
    private function _getReferenceTransaction($sLang, $iOrderNo)
    {
        ...
        if ($this->_aCaptureParams['tid_status'] == '100' && !empty($this->_aCaptureParams['due_date'])) {
        ...
        $sInvoiceComments .= $this->_oLang->translateString('NOVALNET_PAYMENT_REFERENCE_1') . $sData['invoice_ref'];
        $sInvoiceComments .= $this->_oLang->translateString('NOVALNET_PAYMENT_REFERENCE_2') . $sData['tid'];
