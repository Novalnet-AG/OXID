			...
			if ($this->eventData['instalment']['cancel_type'] == 'ALL_CYCLES') {
				...
				for ($dcycle = 1; $dcycle <= $aInstalmentDetails['instalment_total_cycles']; $dcycle++) {
					if (!isset($aInstalmentDetails['instalment' . $dcycle]['tid'])) {
						...
						if (isset($this->eventData['transaction']['refund']['tid']) && !empty($this->eventData['transaction']['refund']['tid'])) {
							$this->aNovalnetComments[] = ['NOVALNET_CALLBACK_REFUND_TID_TEXT' => [$aInstalmentDetails['instalment' . $dcycle]['tid'], NovalnetUtil::formatCurrency($aInstalmentDetails['instalment' . $dcycle]['paid_amount'], $this->eventData['transaction']['currency']) .' '. $this->eventData['transaction']['currency'], $this->eventData['transaction']['refund']['tid']]];
						} else {
							$this->aNovalnetComments[] = ['NOVALNET_CALLBACK_REFUND_TEXT' => [$aInstalmentDetails['instalment' . $dcycle]['tid'], NovalnetUtil::formatCurrency($aInstalmentDetails['instalment' . $dcycle]['paid_amount'], $this->eventData['transaction']['currency']) .' '. $this->eventData['transaction']['currency']]];
