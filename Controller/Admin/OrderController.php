...
class OrderController extends AdminDetailsController
{
            ...
            if (preg_match("/novalnet/i", $oOrder->oxorder__oxpaymenttype->value)) {
                ...
                if ($oOrder->oxorder__oxtotalordersum->value != '0') {
                    if ($aTransDetails['AMOUNT'] != 0) {
                        ...
                        } elseif ($aTransDetails['GATEWAY_STATUS'] == 'CONFIRMED' && $aTransDetails['CREDITED_AMOUNT'] > 0 && $bInstalmentRefund == false && $aTransDetails['PAYMENT_TYPE'] != 'Multibanco') {
                ...
                if (!empty($aResponse['result']['status'])) {
                    if ($aResponse['result']['status'] == 'SUCCESS') {
                        ...
                        if (isset($aResponse['transaction']['refund']['tid']) && !empty($aResponse['transaction']['refund']['tid'])) {
                            $aNovalnetComments[] = ['NOVALNET_CALLBACK_REFUND_TID_TEXT' => [$aResponse['transaction']['tid'], NovalnetUtil::formatCurrency($aResponse['transaction']['refund']['amount'], $aResponse['transaction']['currency']) .' '. $aResponse['transaction']['currency'], $aResponse['transaction']['refund']['tid']]];
                        } else {
                            $aNovalnetComments[] = ['NOVALNET_CALLBACK_REFUND_TEXT' => [$aResponse['transaction']['tid'], NovalnetUtil::formatCurrency($aResponse['transaction']['refund']['amount'], $aResponse['transaction']['currency']) .' '. $aResponse['transaction']['currency']]];
                        }
                        ...
                        if ($sCancelType == 'CANCEL_ALL_CYCLES') {
                            ...
                            for ($dCycle = 1; $dCycle <= $aInstalmentDetails['instalment_total_cycles']; $dCycle++) {
                                if (!isset($aInstalmentDetails['instalment' . $dCycle]['tid'])) {
                                    ...
                                    $aInstalmentDetails['instalment' . $dCycle]['status'] = 'NOVALNET_INSTALMENT_STATUS_REFUNDED';
                                    if (isset($aResponse['transaction']['refund']['tid']) && !empty($aResponse['transaction']['refund']['tid'])) {
										$aNovalnetComments[] = ['NOVALNET_CALLBACK_REFUND_TID_TEXT' => [$aInstalmentDetails['instalment' . $dCycle]['tid'], NovalnetUtil::formatCurrency($aInstalmentDetails['instalment' . $dCycle]['paid_amount'], $aResponse['transaction']['currency']) .' '. $aResponse['transaction']['currency'], $aResponse['transaction']['refund']['tid']]];
									} else {
										$aNovalnetComments[] = ['NOVALNET_CALLBACK_REFUND_TEXT' => [$aInstalmentDetails['instalment' . $dCycle]['tid'], NovalnetUtil::formatCurrency($aInstalmentDetails['instalment' . $dCycle]['paid_amount'], $aResponse['transaction']['currency']) .' '. $aResponse['transaction']['currency']]];
									}
                                }
                            }
                            ...
                            $aAdditionalData['instalment_comments'] = $aInstalmentDetails;
                            $aAdditionalData['novalnet_comments'][] = $aNovalnetComments;
                            NovalnetUtil::updateTableValues('novalnet_transaction_detail', ['ADDITIONAL_DATA' => json_encode($aAdditionalData),'GATEWAY_STATUS' => $aResponse['transaction']['status']], 'ORDER_NO', $aServerResponse['order_id']);
                        } else {
    ...
    protected function updateNovalnetComments($aData, $aResponse)
    {
        ...
        if ($aData['sRequestType'] == 'sOnHold') {
            ...
            $sFormattedAmount = NovalnetUtil::formatCurrency($aResponse['transaction']['refund']['amount'], $oOrder->oxorder__oxcurrency->rawValue) . ' ' .$aResponse['transaction']['currency'];
            if (isset($aResponse['transaction']['refund']['tid']) && !empty($aResponse['transaction']['refund']['tid'])) {
        ...
        } else if ($aData['sRequestType'] == 'sZeroBook') {
