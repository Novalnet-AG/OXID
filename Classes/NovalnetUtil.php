...
class NovalnetUtil {
     ...
     * @var string
     */
    public $sNovalnetVersion = '11.5.0';
     ...
     * @var array
     */
    public $aRedirectPayments =  ['novalnetonlinetransfer', 'novalnetideal', 'novalnetpaypal', 'novalneteps', 'novalnetgiropay', 'novalnetprzelewy24','novalnetalipay','novalnetwechatpay','novalnettrustly','novalnetonlinebanktransfer'];
     ...
     * @var array
     */
    public $aPaymentKey = ['novalnetcreditcard' => 6, 'novalnetsepa' => 37, 'novalnetinvoice' => 27,'novalnetprepayment' => 27, 'novalnetonlinetransfer' => 33, 'novalnetideal' => 49, 'novalnetpaypal' => 34, 'novalneteps' => 50, 'novalnetgiropay' => 69, 'novalnetprzelewy24' => 78, 'novalnetbarzahlen' => 59, 'novalnetalipay' => 115, 'novalnetwechatpay' => 116, 'novalnettrustly' => 110, 'novalnetonlinebanktransfer' => 113];
    ...
    public function doPayment($oOrder)
    {
        ...
        $aNovalnetURL = [     'novalnetcreditcard'  => 'https://payport.novalnet.de/pci_payport',
                              'novalnetonlinetransfer' => 'https://payport.novalnet.de/online_transfer_payport',
                               ...
                               'novalnetalipay'         => 'https://payport.novalnet.de/online_banking_service',
                               'novalnetwechatpay'      => 'https://payport.novalnet.de/online_banking_service',
                               'novalnettrustly'        => 'https://payport.novalnet.de/trustly',
                               'novalnetonlinebanktransfer'        => 'https://payport.novalnet.de/online_banktransfer'
                        ];
        ...
        if (!in_array($this->sCurrentPayment, $this->aRedirectPayments)) {
            ...
            if ($aRequest['payment_type'] == 'PAYPAL' && !empty($aRequest['payment_ref']) && $aResponse['status'] != '100') {
                $this->clearNovalnetRedirectSession();
                $this->oSession->deleteVariable('ordrem');
                $this->oSession->deleteVariable('sess_challenge');
                $this->setNovalnetPaygateError($aResponse);
                return false;
            } else {
                return $aResponse;
            }
    ...
    public function doCurlRequest($mxRequest, $sUrl, $blBuildQuery = true, $blAutoConfig = true)
    {
        ...
        $oCurl->setOption('CURLOPT_TIMEOUT', 240);
    ...
    private function _importUserDetails(&$aRequest, $oUser)
    {
         ...
         $aRequest['remote_ip']   = ($remote_ip == '::1') ? '127.0.0.1' : $remote_ip;
        ...
        if(!empty($oUser->oxuser__oxstateid->value))
            $aRequest['state'] = $oUser->oxuser__oxstateid->value;
     ...
     * @param array &$aRequest
     */
    private function _importPaymentDetails(&$aRequest, $oUser)
    {
        ...
        if ($this->sCurrentPayment == 'novalnetcreditcard') {
            ...
            if (isset($aDynValue['novalnet_cc_new_details']) && $aDynValue['novalnet_cc_new_details'] != '1') {
                ...
                if ($this->getNovalnetConfigValue('blEnforce3D') == '1')
                  $aRequest['enforce_3d'] = '1';
            ...
            if (isset($aDynValue['novalnet_sepa_new_details']) && $aDynValue['novalnet_sepa_new_details'] == '0') {
                ...
                $sFirstName = empty($oUser->oxuser__oxfname->value) ? $oUser->oxuser__oxlname->value : $oUser->oxuser__oxfname->value;
                $sLastName = empty($oUser->oxuser__oxlname->value) ? $oUser->oxuser__oxfname->value : $oUser->oxuser__oxlname->value;
                $aRequest['bank_account_holder'] = $sFirstName.' '.$sLastName;
                $aRequest['iban']                = $aDynValue['novalnet_sepa_iban'];

                if(!empty ($aDynValue['novalnet_sepa_bic']))
                    $aRequest['bic']                = $aDynValue['novalnet_sepa_bic'];
    ...
    private function _importGuaranteedPaymentParameters(&$aRequest)
    {
         if ($this->oSession->getVariable('blGuaranteeEnabled' . $this->sCurrentPayment)) {
            ...
            if ($this->sCurrentPayment == 'novalnetsepa' && (isset($aDynValue['novalnet_sepa_new_details']) && $aDynValue['novalnet_sepa_new_details'] == 0)) {
                $aRequest['birth_date']   = date('Y-m-d', strtotime($aDynValue['birthdate' . $this->sCurrentPayment.'_oneclick']));
            } else {
     ...
     * @param array &$aRequest
     */
    private function _importRedirectPaymentParameters(&$aRequest, $oUser)
    {
        ...
        $aRequest = array_merge($aRequest, [      'implementation' => 'ENC',
                                                  ...
                                                  'input5'         => 'sDeliveryAddressMD5',
                                                  'inputval5'      => $oUser->getEncodedDeliveryAddress(),
                                                  'input6'         => 'nn_sid',
                                                  'inputval6'      => \OxidEsales\Eshop\Core\Registry::getUtilsServer()->getOxCookie('sid'),
    ...
    public function getDueDate()
    {
        ...
        } elseif($this->sCurrentPayment == 'novalnetprepayment') {
            $iDueDate = (!empty($iDueDate) || $iDueDate <= 28 || $iDueDate >= 7)  ? $iDueDate : '';
        }
    ...
    {
        $sErrorMessage = $this->setPaygateError($aResponse);
        $shopSecureHomeURL = $this->oConfig->getShopSecureHomeURL();
        $sPaymentErrorTextParam =  "&payerrortext=". $sErrorMessage;
        $sPaymentErrorParam = '&payerror=-50';
        $sErrorUrl = $shopSecureHomeURL . 'index.php?type=error&cl=payment'. $sPaymentErrorParam . $sPaymentErrorTextParam;
        \OxidEsales\Eshop\Core\Registry::getUtils()->redirect($sErrorUrl, true, 302);
     ...
     * @return string
     */
    public function setPaygateError($aResponse)
    {
    ...
    public function getInvoiceComments($aInvoiceDetails)
    {
        ...
        if ($aInvoiceDetails['tid_status'] == '100' && !empty($aInvoiceDetails['due_date'])) {
        ...
        $sInvoiceComments .= $this->oLang->translateString('NOVALNET_PAYMENT_REFERENCE_2') . $aInvoiceDetails['tid'].'<br>';
    ...
    public function clearNovalnetSession()
    {
        $aNovalnetSessions = [      'sNovalnetAccessKey','aNovalnetGatewayRequest', 'aNovalnetGatewayResponse',
                                    'anovalnetdynvalue', 'nn_aff_id', 'dynvalue', 'blGuaranteeEnablednovalnetsepa',   'blGuaranteeEnablednovalnetinvoice', 'blGuaranteeForceDisablednovalnetsepa', 'blGuaranteeForceDisablednovalnetinvoice'
                                ];
